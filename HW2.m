...
% SS: the built in mean function, when provided a matrix, calculates the row
% wise mean for every column without NaN's. Columns with NA's receive mean's of NAN as well.
%Based on the structure of the meannonan function, seting NaN values to [] takes all the elements of the input, 
%be it a vector or a matrix, evaluates to ensure it is a non-infinite number, and stores as a one dimensional vector. 
%Thus, when a mean is calculated, it is calculated on a vector, providing only one value.
...
G="GEBDYHYTYSG"
[orflength, start_pos, stop_pos]=findORF(seqrand);
[orflength, start_pos, stop_pos]=findORF(G);
...
%
proteinopt=protein2dnaOptimized(test);
